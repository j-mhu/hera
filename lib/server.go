...
func FullShutdown() {
	...
	if err == nil {
		...
		if linescan.Scan() {
			ln := linescan.Text()
			if strings.Contains(ln, "watchdog") || strings.Contains(ln, "mux") {
				pproc, err := os.FindProcess(os.Getppid())
				if err == nil {
					if logger.GetLogger().V(logger.Alert) {
						logger.GetLogger().Log(logger.Alert, "killing parent watchdog", os.Getppid())
					}
					pproc.Kill()
				}
...
func (srv *server) Run() {
	...
	for {
		...
		if srv.startShutdown > 0 {
			...
			if logger.GetLogger().V(logger.Alert) {
				logger.GetLogger().Log(logger.Alert, "watchdog died, terminating")
			}
			return
		}
