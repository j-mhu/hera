...
func (cp *CmdProcessor) ProcessCmd(ns *encoding.Packet) error {
	if ns == nil {
		return errors.New("empty packet passed to processcommand")
	}
		...
		case common.CmdExecute:
			if cp.stmt != nil {
				...
				if cp.result != nil {
					...
					// This entire part will have to be rewritten to follow MySQLPacket code.
				}
				if cp.rows != nil {
					//var cols []string
					...
					// This will probably need to be refactored for MySQLPacket so that we can reconstruct
					// ColumnDefinition and send out binary protocol result sets .. Which requires the encoding
					// to be working when we try to send out Binary Protocol Result sets, prepared statements, etc.
				}
			} else {
				if cp.inTrans {
