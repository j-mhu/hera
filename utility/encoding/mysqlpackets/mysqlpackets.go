...
func NewMySQLPacket(_reader io.Reader) (*encoding.Packet, error) {
	...
	for bytesRead < totalLen {
		n, err = _reader.Read(ns.Serialized[HEADER_SIZE:])
		if err != nil {
			return nil, err
		}
		bytesRead += n
	...
	if bytesRead != totalLen {
		return nil, errors.New(fmt.Sprintf("Expected %d bytes, instead got %d,", totalLen, bytesRead))
	}

	// Read command byte, which is the first byte after the header
	ns.Cmd = int(ns.Serialized[HEADER_SIZE])
	ns.Payload = ns.Serialized[HEADER_SIZE:]
