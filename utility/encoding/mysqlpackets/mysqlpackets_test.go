...
func tmake() ([]nsCase) {
	...
	cases[0] = nsCase{Serialized:query, ns:&encoding.Packet{Cmd:3, Length:18, Sqid:0, Serialized:query, Payload:query[HEADER_SIZE:]}}
	...
	cases[1] = nsCase{Serialized:query, ns:&encoding.Packet{Cmd:22, Length:43, Sqid:0, Serialized:query, Payload:query[HEADER_SIZE:]}}
	...
	cases[2] = nsCase{Serialized:query, ns:&encoding.Packet{Cmd:23, Length:32, Sqid:0, Serialized:query, Payload:query[HEADER_SIZE:]}}
	...
	cases[3] = nsCase{Serialized:query, ns:&encoding.Packet{Cmd:22, Length:32, Sqid:0, Serialized:query, Payload:query[HEADER_SIZE:]}}

	query = []byte{5, 0, 0, 0, 25, 1, 0, 0, 0}
	// payload = []byte{25, 1, 0, 0, 0}
	cases[4] = nsCase{Serialized:query, ns:&encoding.Packet{Cmd:25, Length:5, Sqid:0, Serialized:query, Payload:query[HEADER_SIZE:]}}

	query = []byte{1, 00, 00, 00, 1}
	// payload = []byte{01}
	cases[5] = nsCase{Serialized:query, ns:&encoding.Packet{Cmd:1, Length:1, Sqid:0, Serialized:query, Payload:query[HEADER_SIZE:]}}
...
func TestNewPacketFrom(t *testing.T) {
	...
	for _, tcase := range tcases {
		t.Log("Testing for: ", tcase.Serialized)
		ns := NewMySQLPacketFrom(0, tcase.ns.Serialized[HEADER_SIZE:])
		if ns.Length != tcase.ns.Length {
			t.Log("Length expected", tcase.ns.Length, "instead got", ns.Length)
		}
		if ns.Sqid != tcase.ns.Sqid {
...
func TestPackagerWriteMultiple(t *testing.T) {
	...
	for {
		...
		t.Log("Packet number: ", testPacket.Serialized[3])
		...
		expectedPacket.Sqid++
		expectedPacket.Serialized[3] = expectedPacket.Serialized[3] + 1
