...
func TestReadNext(t *testing.T) {
	...
	for i := 0; i < numPackets; i++ {
		...
		t.Log(expectedPacket.Sequence_id)
	...
	if int(expectedPacket.Sequence_id) != numPackets {
		t.Log("Expected number of packets", numPackets, "instead got", int(expectedPacket.Sequence_id))
		t.Fail()
	}
